...
impl Instance {
    ...
    #[inline(always)]
    pub fn intersects(&self, ray: Ray, t_max: f32) -> Option<(glam::Vec3, glam::Vec3)> {
        let (origin, direction) = ray.into();
        if self
            .bounds
            .intersect(origin, glam::Vec3::one() / direction, t_max)
            .is_none()
        {
            return None;
        }
    ...
    pub fn intersects4(&self, packet: &RayPacket4) -> Option<RayPacket4> {
        ...
        if self
            ...
            .intersect4(
                packet,
                one / glam::Vec4::from(packet.direction_x),
                one / glam::Vec4::from(packet.direction_y),
                one / glam::Vec4::from(packet.direction_z),
            )
            .is_none()
        {
            return None;
